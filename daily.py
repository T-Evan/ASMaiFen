...
class DailyTask:
    ...
    def homePage(self, needQuitTeam=False):
        ...
        while True:
            ...
            if 功能开关["fighting"] == 1 and tryTimes < 5:
                Toast(f'返回首页 - 等待任务结束{tryTimes * 5}/25')
                sleep(5)
                continue
            ...
            # 点击首页-冒险
            re = TomatoOcrTap(330, 1201, 389, 1238, '冒险', sleep1=0.1)
            ...
            for player, messages in player_messages.items():
                team_texts = [
                    ...
                    .replace('晚安~', '').replace('~晚安~', '').replace('~晚安', '').replace('午安~', '').replace(
                        '~午安~', '').replace('~午安', '') for msg in messages
                    if "回环" not in msg]
                if player == '默认':
                    player = ''
                if '旅团' in player:
    ...
    def newMap(self):
        ...
        if 功能开关["自动换图"] == 1:
            ...
            if (not newMapOK and 功能开关["队员不满足单飞"] == 1) or needNewMap:
                ...
                res2 = False
                if not res1:
                    res2, _ = TomatoOcrText(573, 200, 694, 238, "新地图")
                    if not res2:
    ...
    def checkGameStatus(self, needForceCheck=False):
        try:
            ...
            # 战斗中6分钟，强制检查是否卡死
            if not needForceCheck and 功能开关["fighting"] == 1 and (
                    diffTime < 6 * 60 or 任务记录["首页卡死检测-倒计时"] == 0):
                return True
