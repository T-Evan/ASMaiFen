...
class DailyTask:
                ...
                for k in range(100):
                    ...
                    if k > 20:
                        res2 = CompareColors.compare(
                            "615,224,#5D78A6|617,257,#5D78A6|636,257,#5D78A6|667,260,#5D78A6|667,224,#5D78A6")
                        if res2:
                            break
                    if k > 80:
                        Toast(f'返回首页-等待任务{k * 3}/300')
                    sleep(3)
                continue
    ...
    def meiRiXiLian(self):
        ...
        # 判断是否已洗练状态
        res = TomatoOcrTap(625, 363, 709, 388, "冒险手册", 30, -20, sleep1=1)
        if not res:
            Toast("识别冒险手册失败")
            return
    ...
    def shijieShout(self, contentAI=""):
        ...
        if 功能开关['世界AI发言'] == 1 and contentAI == "":
            ...
            for player, messages in player_messages.items():
                ...
                #     return self.shijieShout(random.choice(content))
                blackContent = ['其他区也收', '不想玩的', '茁', '全区收', '收耗', '公众昊']
                ...
                if contains_zan:
                    if zhiye == '奶':
                        content = [f"<color={colors}>{player}~来了~{holidayContent}~{extraContent}</COLOR>",
                                   f"<color={colors}>{player}~来啦~{holidayContent}~{extraContent}</COLOR>"]
                        Toast("世界AI喊话 - 开始")
                        return self.shijieShout(random.choice(content))
                    else:
                        return
                ...
                if contains_zan:
                    if zhiye == '刺' or zhiye == '法' or zhiye == '弓':
                        ...
                        return
                zanList = ['来个ai', '来个AI', '来个Ai', '召唤ai', '召唤AI', '召唤Ai', 'ai打工', '打工ai']
                contains_zan = any(any(zan in text for zan in zanList) for text in team_texts)
                if contains_zan:
                    content = [f"<color={colors}>{player}~来了~{holidayContent}~(*^▽^*)~</COLOR>",
                ...
                contains_zan = any(any(zan in text for zan in zanList3) for text in team_texts)
                if contains_zan:
                    content = [f"<color={colors}>{player}~来了来了~{holidayContent}~{extraContent}</COLOR>",
                               f"<color={colors}>{player}~来了~{holidayContent}~{extraContent}</COLOR>",
                               f"<color={colors}>{player}~来啦~{holidayContent}~{extraContent}</COLOR>"]
                    Toast("世界AI喊话 - 开始")
                    return self.shijieShout(random.choice(content))
                ...
                if contains_zan:
                    content = [f"<color={colors}>{player}~拉我拉我~{extraContent}</COLOR>",
                               f"<color={colors}>{player}~拉我~{holidayContent}~{extraContent}</COLOR>",
        ...
        for content in contents:
            ...
            if 功能开关['世界发言随机字体颜色'] == 1 and "color" not in content:
                colors = generate_random_color()
                content = f"<color={colors}>{content}</COLOR>"
    ...
    def dailyTaskEnd(self):
        ...
        # 使徒来袭
        if 功能开关['使徒来袭'] == 1 and 功能开关['使徒来袭自动接收邀请'] == 0:
            self.ShiTuLaiXi()
        # 箱庭苗圃
    ...
    def newMap(self, forceCheck=False):
        if 功能开关["自动挑战首领"] == 1 and 功能开关["fighting"] == 0:
            re, _ = TomatoOcrText(626, 763, 702, 797, "自动", match_mode='fuzzy')
            if re:
                Toast("日常-挑战首领-等待Boss")
                return
            Toast("日常 - 挑战首领 - 开始")
            self.homePage()
            sleep(1)
        ...
        if 功能开关["自动换图"] == 1:
            ...
            if (not newMapOK and 功能开关["队员不满足单飞"] == 1) or needNewMap:
                Toast('前往下一关-寻找关卡大厅')
                res1, _ = TomatoOcrText(587, 183, 688, 211, "新关卡", match_mode='fuzzy')
                res2 = False
                if not res1:
                    res2, _ = TomatoOcrText(587, 183, 688, 211, "新地", match_mode='fuzzy')
                    if not res2:
                        res2, _ = TomatoOcrText(587, 183, 688, 211, "新关", match_mode='fuzzy')
                if res1 or res2:
                    功能开关["needHome"] = 0
                    功能开关["noHomeMust"] = 1
                    功能开关["fighting"] = 1
                    ...
                    # if 1:
                    # 结伴入口切换
                    res = TomatoOcrTap(645, 434, 691, 457, "结伴", offsetX=10, offsetY=10, sleep1=1.5)
                    if not res:
                    ...
                    for i in range(5):
                        ...
                        功能开关["fighting"] = 1
                        x = 0
                        re, x, y = imageFind('当前地图', confidence1=0.85)
                        if x == 0:
                            re, x, y = imageFind('当前地图2', confidence1=0.85)
                        if x > 0:
        ...
        if 功能开关["单飞后寻找结伴"] == 1 and 功能开关["fighting"] == 0 and not forceCheck:
            if 任务记录["寻找结伴-完成"] == 0:
                Toast("日常 - 寻找结伴 - 开始")
                self.homePage()
                res = TomatoOcrTap(645, 434, 691, 457, "结伴", 10, -10)
                if not res:
    ...
    def ShiTuLaiXi(self):
        for p in range(3):
            ...
            self.homePage()
            self.quitTeam()
            res = TomatoOcrFindRangeClick(keywords=[{'keyword': '使徒', 'match_mode': 'fuzzy'},
                                                    {'keyword': '来袭', 'match_mode': 'fuzzy'}], x1=475, y1=323, x2=634,
                                          y2=683, offsetX=30, offsetY=-20,
                                          sleep1=2.5, match_mode='fuzzy')
            if not res:
                Toast('日常 - 使徒来袭 - 未找到入口')
            ...
            # 领取上把奖励
            re = TomatoOcrTap(331, 718, 386, 744, '开启', offsetX=10, offsetY=10, sleep1=1.5)
            if re:
                Toast('使徒来袭 - 开启上一把宝箱')
            ...
            # 领取全服榜奖励
            re = CompareColors.compare("176,205,#ED5B3E|173,205,#EE5C3E")
            if not re:
                re = CompareColors.compare("176,276,#ED5E42|178,274,#EE5C3E", diff=0.8)
            if re:
                Toast('使徒来袭-领取共享战利品')
                tapSleep(157, 219, 0.8)
                for j in range(6):
            ...
            re = CompareColors.compare("622,692,#F35F42|612,696,#EF5C3F", diff=0.8)
            if re:
                Toast("使徒来袭 - 领取累积奖励")
                tapSleep(544, 738)
                tapSleep(342, 1235)
            ...
            # 识别目标阶段
            toLevel = safe_int_v2(功能开关['使徒来袭目标阶段'])
            if toLevel == 0:
                toLevel = 1700
            if toLevel > 0:
            ...
            self.startFightShiTuLaiXi()

    def startFightShiTuLaiXi(self):
        # 直接开始匹配
        res = TomatoOcrTap(309, 1150, 410, 1178, "开始匹配", 40, -40, sleep1=1)
        if not res:
            Toast("使徒来袭 - 开始匹配失败 - 重试")
        Toast("使徒来袭 - 开始匹配")
        ...
        if res:
            Toast("使徒来袭 - 选择职业")
            if 功能开关["使徒来袭职能优先输出"] == 1:
                tapSleep(280, 665, 0.8)  # 职能输出
            elif 功能开关["使徒来袭职能优先坦克"] == 1:
        ...
        while 1:
            ...
            res1 = self.shilianTask.WaitFight(fightType="使徒来袭")
    ...
    def PaiDuiDaShi(self):
        for p in range(3):
            ...
            self.quitTeam()
            # 开始派对大师
            res1 = TomatoOcrTap(545, 363, 627, 387, "派", 30, -10, match_mode='fuzzy')
            res2 = TomatoOcrTap(545, 363, 627, 387, "舞", 30, -10, match_mode='fuzzy')
            if not res1 and not res2:
                res1 = TomatoOcrFindRangeClick(keywords=[{'keyword': '派', 'match_mode': 'fuzzy'},
                                                         {'keyword': '舞', 'match_mode': 'fuzzy'}], x1=475, y1=323,
                                               x2=634,
                                               y2=683, offsetX=30, offsetY=-20,
    ...
    # 火热的聚会 - 周年活动
    def QiTaQianDaoHuoReJuHui(self):
        if 任务记录["火热的聚会"] == 0:
            Toast('其他签到-火热的聚会-开始')
            # 判断是否在营地页面
            res1 = TomatoOcrTap(11, 1116, 91, 1140, "旅行活动", 40, -20)
            if not res1:
                ...
                if not hd1 and not hd2:
                    Toast('其他签到-火热的聚会-未找到活动入口')
                    return

            isFind = TomatoOcrFindRangeClick(keywords=[{'keyword': '火热', 'match_mode': 'fuzzy'},
                                                       {'keyword': '聚', 'match_mode': 'fuzzy'},
                                                       {'keyword': '会', 'match_mode': 'fuzzy'}], x1=82, y1=61,
                                             x2=653, y2=1153)
            if not isFind:
                # -- 返回活动最后一屏
                self.huoDongSwipeDown()
                ...
                for i in range(1, 5):
                    ...
                    swipe(680, 451, 680, 804)
                    sleep(3)
                    isFind = TomatoOcrFindRangeClick(keywords=[{'keyword': '火', 'match_mode': 'fuzzy'},
                                                               {'keyword': '热', 'match_mode': 'fuzzy'},
                                                               {'keyword': '聚', 'match_mode': 'fuzzy'},
                                                               {'keyword': '会', 'match_mode': 'fuzzy'}], x1=82, y1=61,
                                                     x2=653, y2=1153)
                    if isFind:
            ...
            if isFind:
                ...
                sleep(1)
                for k in range(10):
                    re, _ = TomatoOcrText(480, 741, 565, 776, '已领取')
                    if re:
                        Toast('火热的聚会-日常领取完成')
                        break
                    Toast('火热的聚会-领取日常任务')
                    re = TomatoOcrTap(509, 741, 564, 770, "领取", offsetX=20, offsetY=20, sleep1=0.5)
                    if re:
                        tapSleep(323, 1217)  # 点击空白处关闭
                        tapSleep(323, 1217)  # 点击空白处关闭

                # 领取累积奖励
                re = FindColors.find("226,496,#F35F42|227,496,#F35F41", rect=[97, 468, 626, 571], diff=0.9)
                if re:
                    Toast('火热的聚会-领取累积奖励')
                    tapSleepV2(503, 508)
                    tapSleepV2(435, 508)
    ...
    def CangShuHaoHuo(self):
        if 任务记录["仓鼠好货市集"] == 0:
            ...
            for k in range(3):
                ...
                isFind = TomatoOcrFindRangeClick(x1=88, y1=583, x2=187, y2=1152, offsetX=40,
    ...
    # 免费时装
    def QiTaQianDaoMianFeiShiZhuang(self):
        if 任务记录["免费时装"] == 0:
            self.homePage()
            res = TomatoOcrFindRangeClick(
                keywords=[{'keyword': '免费', 'match_mode': 'fuzzy'},
                          {'keyword': '时装', 'match_mode': 'fuzzy'}], x1=544,
                y1=334, x2=631, y2=623, offsetX=10, offsetY=-5,
                sleep1=2, match_mode='fuzzy')
            if res:
                Toast('免费时装 - 任务开始')
                re = TomatoOcrTap(525, 765, 592, 801, '领取')
                if re:
    ...
    def GaoNengYuJing(self):
        ...
        res = TomatoOcrFindRangeClick('EVA', x1=544, y1=334, x2=631, y2=623, offsetX=30, offsetY=-20,
                                      sleep1=1.5, match_mode='fuzzy')
        if res:
            Toast('EVA-高能预警-任务开始')
    ...
    # 明灯还年
    def QiTaQianDaoMingDeng(self):
        if 任务记录["明灯还年"] == 0:
            self.homePage()
            # 寻找花灯
            re = CompareColors.compare("353,849,#F6F2E9|375,850,#F7F2E9|405,850,#F7F2E9", diff=0.8)
            if re:
                Toast('周年庆典 - 明灯还年 - 寻找熄灭的花灯')
                tapSleep(268, 899, 2)
                for k in range(10):
            ...
            if res:
                ...
                if re:
                    ...
                    # re, _ = TomatoOcrText(265,618,438,659, '花灯',match_mode='fuzzy')
                    # if re:
                    #     Toast('周年庆典 - 明灯还年 - 任务开始')

                re, _ = TomatoOcrText(268, 620, 372, 653, '占卜时刻')
                if re:
                    Toast('明灯还年 - 寻找占卜时刻')
                    tapSleep(154, 1199)
                    tapSleep(154, 1199)
                    tapSleep(154, 1199)
                    tapSleep(462, 675, 3.5)  # 技能商店
                    re, _ = TomatoOcrText(624, 181, 706, 211, '解', match_mode='fuzzy')
                    if re:
                        re = CompareColors.compare("451,265,#FBF5EA|443,266,#FBF5EA|434,266,#FBF5EA")
                        if re:
                            tapSleep(364, 321, 2)
                            for k in range(10):
                                tapSleep(328, 1221)  # 点击空白
                            tapSleep(104, 1221)  # 点击返回

                re, _ = TomatoOcrText(268, 620, 372, 653, '哪吒')
                if re:
                    Toast('明灯还年 - 寻找哪吒')
                    tapSleep(154, 1199)
                    tapSleep(55, 97, 1)  # 点击头像
                    re, _ = TomatoOcrText(540, 1060, 593, 1087, '养成')
                    if re:
                        tapSleep(394, 969, 2)
                        for k in range(10):
    ...
    def QiTaQianDao(self):
        ...
        self.CangShuHaoHuo()
        # 明灯还年
        # self.QiTaQianDaoMingDeng()
        # 免费时装
        self.QiTaQianDaoMianFeiShiZhuang()
        # 欢庆连五日
        # if 任务记录["欢庆连五日"] == 0:
        #     Toast('欢庆连五日 - 开始')
        #     # 返回首页
        ...
        if 任务记录["忆战回环签到"] == 0:
            ...
            if res:
                ...
                if not res:
                    res = CompareColors.compare("628,110,#F45F42|625,113,#F05D40")
                if res:
                    Toast('忆战回环签到 - 任务开始')
        ...
        if 任务记录["拾光奇旅"] == 0:
            ...
            res, _ = TomatoOcrText(11, 1116, 91, 1140, "旅行活动")
            if res:
                res = TomatoOcrFindRangeClick(keywords=[{'keyword': '拾光', 'match_mode': 'fuzzy'},
                                                        {'keyword': '奇旅', 'match_mode': 'fuzzy'}], match_mode='fuzzy',
                                              x1=96, y1=550, x2=181, y2=1060, offsetX=30,
                                              offsetY=-20,
                                              sleep1=0.8)
                if res:
                    Toast('拾光奇旅 - 任务开始')
        ...
        if 任务记录["夜空流星"] == 0:
            ...
            res, _ = TomatoOcrText(11, 1116, 91, 1140, "旅行活动")
            if res:
                res = TomatoOcrFindRangeClick(keywords=[{'keyword': '夜空', 'match_mode': 'fuzzy'},
                                                        {'keyword': '流星', 'match_mode': 'fuzzy'}], match_mode='fuzzy',
                                              x1=96, y1=550, x2=181, y2=1060, offsetX=30,
                                              offsetY=-20,
                                              sleep1=0.8)
                if res:
        ...
        # 珍珠之河
        if 任务记录["珍珠之河"] == 0:
            self.fuZhuangLingQu(taskName='珍珠', searchName='珍珠')
            任务记录["珍珠之河"] = 1

        # 怒莲风尚
        if 任务记录["怒莲风尚"] == 0:
            self.fengShangLingQu(taskName='怒莲', searchName='怒莲')
            任务记录["怒莲风尚"] = 1
    ...
    def fengShangLingQu(self, taskName='', searchName=''):
        ...
        if res:
            ...
            res = TomatoOcrTap(304, 1022, 348, 1047, "免费", sleep1=2)
            res = TomatoOcrTap(304, 1018, 352, 1047, "免费", sleep1=2)
            tapSleep(323, 1232, 1)
            tapSleep(323, 1232, 1)
            tapSleep(323, 1232, 1)
            ...
            if 功能开关['钻石购买服装币'] == 1:
                ...
                if zuanShi >= 1800:
                    re = TomatoOcrTap(545, 1052, 626, 1075, '特惠商城', offsetX=20, offsetY=-40)
                    if re:
                        re = TomatoOcrTap(157, 653, 222, 681, '1800')
                        if re:
    ...
    def XiangTingMiaoPu(self):
        ...
        if 功能开关["箱庭苗圃自动提交任务"] == 1:
            re = CompareColors.compare("674,160,#F86142|675,167,#F45D42", diff=0.8)
            if not re:
                re = CompareColors.compare("680,164,#F45F42|683,165,#F45E42")
            if re:
                Toast('箱庭苗圃 - 提交任务')
                tapSleep(680, 168, 2)
                for m in range(13):
                    ...
                    if re:
                        ...
                        action.Touch.down(131, 282, 4000)
                        action.Touch.up(131, 282, 4000)
                        action.Touch.down(222, 280, 4000)
                        action.Touch.up(222, 280, 4000)
        ...
        if 功能开关["箱庭苗圃自动兑换商品"] == 1:
            for k in range(4):
                ...
                if coin >= 10:
                    ...
                    if re:
                        res, tmpText = TomatoOcrText(x - 20, y + 130, x + 60, y + 150, "已", match_mode='fuzzy')
    ...
    def baoZangHu(self):
        ...
        # 展开更多
        re = CompareColors.compare("53,678,#FFFFFF|71,677,#F15E41")
        if re:
            tapSleep(52, 683, 1.2)

        res = TomatoOcrFindRangeClick(
            keywords=[{'keyword': '宝', 'match_mode': 'fuzzy'}, {'keyword': '藏', 'match_mode': 'fuzzy'},
                      {'keyword': '湖', 'match_mode': 'fuzzy'}], sleep1=0.9, x1=15, y1=672, x2=93, y2=1149, offsetX=30,
            offsetY=-20)
        if not res:
            return
    ...
    def jinJiWeiTuo(self):
        ...
        # 准备挖掘
        res = TomatoOcrTap(457, 1141, 576, 1175, "备", 10, 10, match_mode='fuzzy')
        if res:
            res = TomatoOcrTap(457, 1141, 576, 1175, "备", 10, 10, match_mode='fuzzy')
            res = TomatoOcrTap(457, 1141, 576, 1175, "备", 10, 10, match_mode='fuzzy')
            Toast('准备挖掘')
        ...
        for l in range(3):
            ...
            re4 = FindColors.find("584,1023,#FFFFFF|592,1025,#FFFFFF|592,1025,#FFFFFF|588,1037,#FFFFF9",
                                  rect=[80, 515, 648, 1090], diff=0.95)
            if not re4:
                re4 = FindColors.find(
        ...
        if coin > 0:
            # 自动剪彩
            res = TomatoOcrTap(457, 1142, 574, 1175, "自动", 10, 10, match_mode='fuzzy')
            if res:
                Toast('开始剪彩')
                sleep(10)
            ...
            #
            # # 自动挖掘
            # res = TomatoOcrTap(462, 1144, 573, 1177, "自动", 10, 10, match_mode='fuzzy')
            # if res:
            #     Toast('自动挖掘')
            #     sleep(10)
        else:
    ...
    def huoDongDiaoYu(self):
        ...
        if not res:
            self.homePage()
            TomatoOcrTap(125, 1202, 187, 1234, "营地", sleep1=1.5)
        # 判断是否在营地页面
        res = TomatoOcrFindRangeClick("钓鱼", match_mode='fuzzy', x1=97, y1=544, x2=180, y2=1051, offsetX=30,
                                      offsetY=-20,
                                      sleep1=3.5)
        if res:
            Toast(f'钓鱼王 - 任务开始')
            # 领取任务奖励
            re = CompareColors.compare("72,381,#F35E41|74,378,#FA6244")  # 任务红点
            if re:
                tapSleep(72, 385, 0.8)
                TomatoOcrTap(495, 303, 547, 334, '领取')
                tapSleep(503, 1043)
                re = CompareColors.compare("595,1077,#F55F42|598,1079,#EF5C40", diff=0.8)  # 活动任务红点
                if re:
                    tapSleep(541, 1090, 1.5)  # 点击活动任务
                    TomatoOcrTap(495, 303, 547, 334, '领取')
                    TomatoOcrTap(495, 303, 547, 334, '领取')
                tapSleep(345, 1238)  # 返回
                tapSleep(345, 1238)
                tapSleep(345, 1238)

            # 识别剩余鱼饵
            res, availableCount = TomatoOcrText(593, 82, 674, 102, "鱼饵")
            availableCount = safe_int_v2(availableCount)
            if availableCount == 0:
                Toast(f'钓鱼王 - 剩余鱼饵数量为0')
            if availableCount > 0:
                for k in range(10):
                    re = TomatoOcrTap(528, 1059, 591, 1092, '出杆')
                    if re:
                        sleep(1.5)
                        tapSleep(558, 1073, 4)  # 确认出杆
                    for p in range(30):
                        if p > 5:
                            re = CompareColors.compare("251,448,#37536B|268,452,#37536B|284,451,#F1F1F1")  # 长按出杆
                            if not re:
                                re = CompareColors.compare("450,408,#48618E|456,408,#48618E")
                                if not re:
                                    break
                        action.Touch.down(558, 1044, 1400)  # 长按
                        action.Touch.up(558, 1044, 1400)
                        sleep(0.7)

                    tapSleep(345, 1238)  # 点击空白
                    tapSleep(345, 1238)
                    ...
                    if availableCount == 0:
                        ...
                        break
        任务记录["钓鱼王"] = 1
        TomatoOcrTap(77, 1191, 126, 1215, '返回')
        ...
        for j in range(2):
            ...
            for i in range(1, 3):
                ...
                if not res1 and not res2:
                    ...
                    res = TomatoOcrTap(125, 1202, 187, 1234, "营地")
                    # 判断是否在营地页面
                    hd1, _ = TomatoOcrText(11, 1116, 91, 1140, "旅行活动")
            ...
            if res2:
                ...
                if needCount > 0:
                    for i in range(5):
                        res = TomatoOcrTap(541, 262, 619, 288, "兑换门票", 15, -15, sleep1=0.5)
    ...
    def zhaoShiChuangZao(self):
        ...
        for j in range(2):
            ...
            if needCount > 0:
                for i in range(1, 5):
                    res = TomatoOcrTap(541, 262, 619, 288, "兑换卢恩", 15, -15, sleep1=0.8)
                    buyCount = ""
                    for p in range(1, 5):
            ...
            if 功能开关["自动讲述领取绮想妙成真"] == 0 or (
                    功能开关["自动讲述领取绮想妙成真"] == 1 and "次" not in availableCount):
    ...
    def quitTeam(self):
        ...
        if res1 or res2 or res3 or res4 or res5 or res6:
            ...
            if not teamExist:
                teamExist = TomatoOcrFindRangeClick(
                    keywords=[{'keyword': '离', 'match_mode': 'fuzzy'}, {'keyword': '开', 'match_mode': 'fuzzy'},
                              {'keyword': '队', 'match_mode': 'fuzzy'}, {'keyword': '伍', 'match_mode': 'fuzzy'}],
                    whiteList='离开队伍', x1=416, y1=126, x2=628, y2=284)
            if teamExist:
    ...
    def shopBuy(self, bugType=''):
        ...
        if not re1:
            re1, _ = TomatoOcrText(334, 803, 385, 828, '购买')
        re2 = False
        if re1:
            ...
            if re:
                ...
                if not re:
                    re = TomatoOcrTap(334, 803, 385, 828, '购买', offsetX=5, offsetY=5)
                if not re:
