...
特征 = {
    ...
    # '首页-冒险': ImageQuery().find().rect_half_bottom().img('shouye_maoxian.png'),
    ...
    '首页-挑战首领': ImageQuery().find().rect_half_right().img('挑战首领.png'),
    '首页-新关卡已解锁': OcrQuery().paddleocr_v3().rect_half_top().rect_half_right().pattern('已解锁'),
    # '首页-营地': ImageQuery().find().rect_half_bottom().img('首页-营地.png'),

    '营地-旅行活动': OcrQuery().mlkitocr_v2().rect_half_left().rect_half_bottom().pattern('旅行活动'),
    '营地-仓鼠百货': ImageQuery().find().rect_half_top().img('仓鼠百货.png'),
    '营地-仓鼠百货2': ImageQuery().find().rect_half_top().img('仓鼠百货2.png'),
    ...
    '仓鼠百货-购买': ImageQuery().find().img('商店购买.png'),
    '仓鼠百货-商品已售罄': OcrQuery().mlkitocr_v2().pattern('商品已售罄'),
    '仓鼠百货-星星经验': ImageQuery().find().img('星星经验.png'),
    '仓鼠百货-全价兽粮': ImageQuery().find().img('全价兽粮.png'),
    '仓鼠百货-超级成长零食三折': ImageQuery().find().img('超级成长零食三折.png'),
    '仓鼠百货-黑烬突破石五折': ImageQuery().find().img('黑烬突破石五折.png'),
    '仓鼠百货-经验补剂五折': ImageQuery().find().img('经验补剂五折.png'),
    '营地-秘宝': ImageQuery().find().rect_half_top().img('营地秘宝.png'),
    '招式创造-能量': ImageQuery().find().rect_half_top().img('招式创造能量.png'),
    '营地-月签到': ImageQuery().find().img('月签到.png'),
    '营地-纸飞机': ImageQuery().find().img('纸飞机.png'),
    '活动-登录好礼': ImageQuery().find().img('登录好礼.png'),
    '活动-登录好礼-领取': ImageQuery().find().img('活动-登录好礼-领取.png'),
    '活动-摸鱼': OcrQuery().paddleocr_v3().rect_half_right().pattern('摸鱼'),
    '活动-摸鱼中': ImageQuery().find().img('摸鱼中.png'),
    '秘宝-能量': ImageQuery().find().img('秘宝能量.png'),
    # '秘宝-能量': OcrQuery().paddleocr_v3().pattern('x100'),
    # '秘宝-加号': ImageQuery().find().rect_half_top().rect_half_right().img('秘宝加号.png'),
    '秘宝-补充能源': OcrQuery().paddleocr_v3().rect_half_top().rect_half_right().pattern('补充能源'),
    '秘宝-暗月深林': ImageQuery().find().img('暗月深林.png'),
    '秘宝-艾特拉火山': ImageQuery().find().img('艾特拉火山.png'),
    '秘宝-鲁尔绿洲': ImageQuery().find().img('鲁尔绿洲.png'),
    '秘宝-燃烧塔': ImageQuery().find().img('燃烧塔.png'),
    '结伴-当前地图': ImageQuery().find().img('当前地图.png'),
    '结伴-当前地图2': ImageQuery().find().img('当前地图2.png'),
    '结伴-组队增益': ImageQuery().find().img('组队增益.png'),
    '结伴-加入': OcrQuery().paddleocr_v3().pattern('加入'),
    '手册-领取': ImageQuery().find().img('手册-领取.png'),

    '旅人-一键强化': ImageQuery().find().img('一键强化.png'),
    '旅人-技能升级': ImageQuery().find().img('技能升级.png'),
    '旅人-最大': OcrQuery().mlkitocr_v2().pattern('最大'),
    '旅人-猫猫果木': ImageQuery().find().img('猫猫果木.png'),

    '旅团-任务': OcrQuery().paddleocr_v3().pattern('旅团任务'),
    '旅团-捐献': OcrQuery().paddleocr_v3().pattern('捐献'),
    '旅团-领取': OcrQuery().paddleocr_v3().pattern('领取'),
    '旅团-唤兽琴弦': ImageQuery().find().img('旅团-唤兽琴弦.png'),
    '旅团-全价兽粮': ImageQuery().find().img('旅团-全价兽粮.png'),
    '旅团-超级成长零食': ImageQuery().find().img('旅团-超级成长零食.png'),
    '旅团-原材料': ImageQuery().find().img('旅团-原材料.png'),
    '旅团-史诗经验': ImageQuery().find().img('旅团-史诗经验.png'),
    '旅团-优秀经验': ImageQuery().find().img('旅团-优秀经验.png'),
    '旅团-普通经验': ImageQuery().find().img('旅团-普通经验.png'),
    '旅团-金币': ImageQuery().find().img('旅团-金币.png'),
    '旅团-最大': OcrQuery().paddleocr_v3().pattern('最大'),
    '旅团-购买': OcrQuery().paddleocr_v3().pattern('购买'),

    '试炼-秘境之间': ImageQuery().find().img('秘境之间.png'),
    '秘境-创建队伍': ImageQuery().find().img('秘境-创建队伍.png'),
    ...
    '试炼-恶龙大通缉': ImageQuery().find().img('恶龙大通缉.png'),
    '恶龙-开始匹配': OcrQuery().paddleocr_v3().pattern('开始匹配'),
    '恶龙-宝箱已开启': ImageQuery().find().img('恶龙-宝箱已开启.png'),
    ...
    '战斗中-喊话': ImageQuery().find().rect_half_right().img('战斗-喊话.png'),
    ...
    '战斗结束-点赞': ImageQuery().find().rect_half_top().img('dianzan.png'),
    '战斗结束-开启宝箱': ImageQuery().find().rect_half_bottom().img('宝箱-开启.png'),
    '战斗结束-自动准备': OcrQuery().mlkitocr_v2().rect_half_bottom().pattern('自动准备'),  # 战斗结束页，自动准备提示 -- 快速返回房间
    '战斗结束-点赞队友1': ImageQuery().find().rect_half_top().img('点赞1.png'),
    '战斗结束-点赞队友2': ImageQuery().find().rect_half_top().img('点赞2.png'),

    '首页-正在组队': OcrQuery().mlkitocr_v2().rect_half_right().pattern('^正在组队$'),
    '首页-匹配中': OcrQuery().mlkitocr_v2().rect_half_right().pattern('^匹配中$'),
    '队伍-匹配中': OcrQuery().mlkitocr_v2().rect_half_bottom().pattern('^匹配中$'),
    ...
    '返回-2': ImageQuery().find().rect_half_left().rect_half_bottom().img('返回_1.png'),
    '返回-3': OcrQuery().mlkitocr_v2().rect(7, 1066, 216, 1273).pattern('回'),
    '返回-4': ImageQuery().find().rect_half_left().rect_half_bottom().img('返回_2.png'),
