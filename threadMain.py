...
from .child_kami import main as check_kami
...
class RunThreadCheckKami(dynamic_proxy(Runnable)):
    def __init__(self):
        super().__init__()
...
def runThreadCheckKami():
    try:
        r = RunThreadCheckKami()
        t = Thread(r)
        t.start()
    except RuntimeError as e:
        print(f"设备心跳处理线程 Error: {e}")
