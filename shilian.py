...
class ShiLianTask:
    ...
    def sanMoTou(self):
        ...
        for k in range(3):
            res = TomatoOcrFindRangeClick(
                keywords=[{'keyword': '三', 'match_mode': 'fuzzy'}, {'keyword': '魔', 'match_mode': 'fuzzy'},
                          {'keyword': '头', 'match_mode': 'fuzzy'}, {'keyword': '打', 'match_mode': 'fuzzy'},
                          {'keyword': '守', 'match_mode': 'fuzzy'}],
                x1=552,
                y1=333, x2=626, y2=512, offsetX=30,
                offsetY=-10, sleep1=1)
            if res:
                break
        if not res:
        ...
        # 领取全服心火奖励
        re = FindColors.find("610,437,#F56042|613,438,#F55F42|611,440,#F05D40", rect=[563, 344, 630, 756], diff=0.9)
        if re:
            tapSleep(re.x, re.y)
    ...
    def zhiGuZhiXing(self):
        ...
        for p in range(3):
            ...
            self.quitTeam()
            # 开始桎梏之形
            res = TomatoOcrTap(544, 460, 631, 487, "桔之形", 30, -10)  # 适配新手试炼 - 下方入口
            if not res:
                res = TomatoOcrTap(544, 460, 631, 487, "桎梏之形", 30, -10)  # 适配新手试炼 - 下方入口
            if not res:
                res = TomatoOcrTap(556, 380, 618, 404, "桎梏之形", 30, -10)
            if not res:
        ...
        for k in range(8):
            ...
            re = FindColors.find("284,696,#F05E40|286,693,#F46042|286,698,#F45A40")  # 匹配红点
            if re:
                Toast("桎梏之形 - 领取累积奖励")
                tapSleep(re.x - 30, re.y + 30)
                tapSleep(125, 1050)  # 领取后，点击空白
                tapSleep(125, 1050)  # 领取后，点击空白
    ...
    def changeMap(self, selectMap, selectStage):
        ...
        for k in range(4):
            ...
            # 判断是否切换成功
            res, _, _ = TomatoOcrFindRange(selectMap, x1=80, y1=214, x2=637, y2=599, match_mode='fuzzy')
            if res:
                return True
        Toast(f'切换地图失败')
        return False
    ...
    def openTreasure(self, noNeedOpen=0):
        ...
        res1 = FindColors.find(
            "181,1014,#F3A84B|185,1030,#F3A84B|211,1040,#F3D669|233,1037,#FBF8F7|263,1024,#F3A84B|289,1040,#F3A84B",
            rect=[77, 175, 636, 1164], diff=0.95)
        if not res1:
            res1, tmp1 = TomatoOcrText(514, 607, 592, 634, "战斗统计")  # 战斗结束页。宝箱提示
            if not res1:
                res1, tmp1 = TomatoOcrText(514, 517, 592, 542, "战斗统计")  # 战斗结束页。宝箱提示
                if not res1:
                    res1, tmp2 = TomatoOcrText(510, 547, 592, 572, "一键全赞")  # 战斗结束页。宝箱提示
                    if not res1:
                        res1, tmp2 = TomatoOcrText(511, 458, 595, 484, "一键全赞")  # 战斗结束页。宝箱提示
                    #     if not res1:
    ...
    def fightingDouGeHuiTeam(self):
        ...
        while 1:
            if elapsed >= totalWait:
                Toast("战斗结束 - 斗歌会超时退出组队")
                self.teamShoutAI(f'斗歌会-即将离队-期待下次相遇', shoutType="fight")
                功能开关["fighting"] = 1
                sleep(2)
            ...
            if "等级" in teamName1 or "等级" in teamName2 or "Lv" in teamName1 or "Lv" in teamName2:
                功能开关["fighting"] = 1
                功能开关["needHome"] = 0
                Toast(f'斗歌会战斗中,战斗时长{elapsed}/{totalWait}秒')
                if teamShoutDone == 0:
                    teamName = 任务记录['战斗-房主名称']
                    teamCount = 任务记录['带队次数']
                    self.teamShoutAI(
                        f'斗歌会-留镜像后离队~祝你武运昌隆~{teamName}-第{teamCount}次相遇~祝你游戏开心~',
                        shoutType="fight")
                    teamShoutDone = self.teamShout()
                self.AIContent()
            else:
                # 战斗结束
                res1 = TomatoOcrTap(334, 1060, 385, 1087, "开启", offsetX=10, offsetY=10)  # 领取宝箱
                res2 = TomatoOcrTap(334, 1060, 385, 1089, "开启", offsetX=10, offsetY=10)  # 领取宝箱
                res3 = TomatoOcrTap(333, 711, 383, 737, "开启")  # 领取宝箱
                res4 = CompareColors.compare(
                    "448,803,#FFE298|445,782,#F8D37E|448,770,#FBD682|426,770,#D0714C|483,763,#9C3B43")  # 匹配宝箱颜色
                res5 = TomatoOcrText(516, 861, 593, 885, '战斗统计')
            ...
            if elapsed > 180 or (
                ...
                Toast(f"斗歌会战斗中状态 - 识别失败 - 次数 {failNum}/4")
                failNum = failNum + 1
                if failNum > 4:
                    Toast(f"斗歌会战斗中状态 - 识别失败 - 退出战斗")
                    break
                if failNum > 7:
        ...
        while 1:
            ...
            if elapsed >= totalWait:
                ...
                if res:
                    if 功能开关['秘境点赞队友'] == 1:
                        Toast('点赞队友')
                        res = TomatoOcrTap(508, 784, 592, 804, "一键全赞", 5, 5)  # 一键点赞
                        if not res:
                            res = TomatoOcrFindRangeClick("全赞", x1=480, y1=490, x2=615, y2=768,
                                                          match_mode='fuzzy')  # 一键点赞
        ...
        while 1:
            ...
            if elapsed >= totalWait:
                ...
                if res:
                    if 功能开关['秘境点赞队友'] == 1:
                        Toast('点赞队友')
                        res = TomatoOcrTap(515, 513, 593, 534, "一键全赞", 5, 5)  # 一键点赞
                        if not res:
                            res = TomatoOcrFindRangeClick("全赞", x1=480, y1=490, x2=615, y2=768,
                                                          match_mode='fuzzy')  # 一键点赞
